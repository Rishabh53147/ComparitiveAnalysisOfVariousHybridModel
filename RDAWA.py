...
# Load the data
data = pd.read_csv("INFY_NS.csv")  # Replace with your dataset
...
# Convert the predicted prices to the original scale
predicted_prices_original_scale = np.array(predicted_prices)

# Check if the predicted prices are within a tolerance of the true prices
tolerance = 0.02  # Adjust this value based on the scale of your stock prices
correct_predictions = np.sum(np.abs(predicted_prices_original_scale - stock_prices[window_size:]) <= tolerance)
...
plt.plot(predicted_prices_original_scale, label='Predicted Prices', color='red')
